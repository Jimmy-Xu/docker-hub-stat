...
IDX=0
echo -n > etc/tag_tofetch.lst #clear list
...
do
  ...
  layer_d=${f/.json/}
  layer_d=${layer_d/\/tags\//\/layers\/}
  #[ ! -d ${layer_d} ] && mkdir -p ${layer_d}
  repo_name=${layer_d/result\/layers\//}
...
# EOF
  ...
  if [ "${TAG_RANGE}" == "all" ];then
    ...
    do
      ...
      if [ ! -s ${layer_f} ];then
        echo ${repo_name} ${l} >> etc/tag_tofetch.lst
      else
...
# start fetch image tag from etc/tag_tofetch.lst
JOB_TOTAL=0

TOTAL_TOFETCH=$(cat etc/tag_tofetch.lst|wc -l )
...
do
  ...
  {
    #exec job
    RLT_PATH=${WORKDIR}/result/layers/${CURRENT_REPO}
    mkdir -p ${RLT_PATH}
